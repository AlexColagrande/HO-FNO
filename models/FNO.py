...
class FNOConv(nn.Module):
    """FNO spectral branch using the same diagonal spectral convolution as HO-FNO."""
...
class FNOBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.fno_conv = FNOConv(width, width, modes1, modes2)
        self.norm1 = RMSNorm2D(width, name1)
        self.norm2 = RMSNorm2D(width, name2)
        self.MLP = nn.Sequential(
            nn.Linear(width, width * expansion_MLP),
            nn.GELU(),
        ...
        self.fno_blocks = nn.ModuleList(
            [
                FNOBlock(
                    ...
                    modes1,
                    modes2,
                    name1=f"norm pre conv {layer}",
                    name2=f"norm pre mlp {layer}",
                    p_drop_MLP=p_drop_MLP,
                    drop_path=drop_rates[layer],
                    expansion_MLP=expansion_MLP,
                    two_thirds_zero_padding=two_thirds_zero_padding,
                )
